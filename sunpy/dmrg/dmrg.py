...
class DMRG:
    ...
    def __init__(self, N, Ns, num_irreps, max_num_states, **kwargs):
        ...
        self._do_check = True
        self._test_Ns_threshold = int(8) # tests will be performed up to this length (half-chain)
        
        if self._N==3:
            self._irreps_all = np.load('sunpy/irreps/SU3_irreps_300.npy').astype(int)
        else:
        ...
        if 'rme_filename' in kwargs:
            ...
            self._rme_filename = 'pythonRME_fund_SU' + str(self._N) + '_' + self._target + '_numirreps' + str(num_irreps) + '.pickle'
            self._rme_filename = os.path.join(os.getcwd(), 'sunpy', 'rme_coefficients', self._rme_filename)
