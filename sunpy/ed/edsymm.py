...
class EDSolverSymm(EDSolver):
    """
    Class to represent an exact diagonalization solver for SU(N) Heisenberg 
    models with m particles per site in the symmetric m-box representation
    """
    ...
    def __init__(self, N: int, Ns: int, alpha, m: int, lattice: Lattice):
        ...
        if not self._n==self._Ns*self._m:
            sys.exit('ERROR : EDEngineSymm : __init__ : number of boxes in alpha must match Ns*m')
        
        if self._m>3:
            sys.exit('ERROR : EDEngineSymm : __init__ : Code not yet implemented for m>3')
