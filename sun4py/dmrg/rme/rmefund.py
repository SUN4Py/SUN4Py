...
from sun4py.dmrg.rme.rmebase import RMEEngine
from sun4py.dmrg.rme.utils import states_rme
from sun4py.dmrg.rme.rmereader import RMEReader
import sun4py.common.math
from sun4py.sun import sun
from sun4py.sdc.dmrg import sdcdmrgbase # basic method (slow)
from sun4py.sdc.dmrg import sdcdmrg # shortcut method ("fast")



class RMEEngineFund(RMEEngine):
    """
    Class to compute Reduced Matrix Elements (RMEs) of the Heisenberg interaction
    for the fundamental irrep at each site
    """
    ...
    def __init__(self, N, num_irreps, target, tech, **kwargs):
        ...
        rme_folder : str [default: 'path/to/user/script/sun4py_rmes/']
            path to directory containing files of RMEs
        ...
        if not target in ['GS']:
            raise ValueError('RMEEngineFund : __init__ : target sector undefined')
    ...
    def _target_irrep(self, n):
        ...
        if self._target=='GS':
            ...
            raise ValueError('RMEEngineFund : _target_irrep: Target irrep undefined.')
    ...
    def _atomic_run(self):
        ...
        for p in range(0, num_states):
            ...
            if (mult_1>1):
                sys.exit('ERROR : RMEEngineFund : _atomic_run : Problem: mult_1>1')
                ...
                for d1 in range(0, nbd_1):
                    for d2 in range(0, nbd_2):
                        ...
                        if (mult_2>1):
                            sys.exit('ERROR : RMEEngineFund : _atomic_run : Problem: mult_2>1')
                            ...
                            if self._restarting:
                                ...
                                ind1_old = sun4py.common.math.find_row(self._irreps_old, alpha1)
                                ind2_old = sun4py.common.math.find_row(self._irreps_old, alpha2)
                                ind3_old = sun4py.common.math.find_row(self._irreps_old, alpha3p)
                                ind4_old = sun4py.common.math.find_row(self._irreps_old, alpha4p)
                                
                                if len(ind1_old)*len(ind2_old)*len(ind3_old)*len(ind4_old)==1:
                                    doCalc = False
                            
                            ind3 = sun4py.common.math.find_row(self._irreps, alpha3p)
                            ind4 = sun4py.common.math.find_row(self._irreps, alpha4p)
                            ...
                            if ((len(ind3)>0) & (len(ind4)>0)):
                                ...
                                index_bra = sun4py.common.math.find_row(states, bra)[0]
                    ...
                    for i in range(0, len(rme_to_compute)):
                        ...
                        if td['doCalc']==True:
                            if (td['index_bra']<p):                                
                                ...
                                _, bra_ind, Pket_ind = sun4py.common.math.intersect_row(bra_cy, Pket_cy)
